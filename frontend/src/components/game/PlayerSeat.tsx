...
export function PlayerSeat({
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div
          className={clsx(
            ...
            isCurrentPlayer ? 'bg-gradient-to-br from-blue-500 to-blue-700' : 'bg-gradient-to-br from-gray-600 to-gray-800',
            isCurrentTurn && 'ring-4 ring-yellow-400 shadow-[0_0_20px_rgba(251,191,36,0.6)]'
          )}
        >
        ...
        {/* Current bet badge */}
        {player.bet > 0 && (
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-yellow-500 text-black rounded-full text-xs font-bold whitespace-nowrap shadow-md">
            ${player.bet}
          </div>
        )}
      </div>
      ...
      {/* Status indicators */}
      {player.folded && (
        <div className="mt-1 px-2 py-0.5 bg-red-600 rounded text-white text-xs font-bold">FOLD</div>
      )}
      {player.isAllIn && (
        <div className="mt-1 px-2 py-0.5 bg-yellow-500 rounded text-black text-xs font-bold">ALL IN</div>
      )}
    </div>
    </div>
  );
