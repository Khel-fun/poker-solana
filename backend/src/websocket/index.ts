...
export function setupWebSocket(io: GameServer): void {
  io.on('connection', (socket: GameSocket) => {
    ...
    socket.on('join_game', ({ gameId, player }) => {
      ...
      if (!result.success) {
        socket.emit('error', { message: result.error || 'Failed to join game', code: result.error || 'UNKNOWN' });
        return;
      }
      ...
      const joinedPlayer = result.game!.players.find(p => p.id === player.id)!;
      socket.to(gameId).emit('player_joined', joinedPlayer);
      
      // Send full game state to the joining player
      const gameState = GameService.getGameStateForPlayer(result.game!, player.id);
    ...
    socket.on('start_game', ({ gameId }) => {
      ...
      if (!result.success) {
        socket.emit('error', { message: result.error || 'Failed to start game', code: result.error || 'UNKNOWN' });
        return;
      }
      ...
      for (const player of result.game!.players) {
        ...
        if (playerSocketId) {
          const playerState = GameService.getGameStateForPlayer(result.game!, player.id);
      ...
      const validActions = GameService.getValidActions(result.game!, currentPlayer);
    ...
    socket.on('player_action', ({ gameId, action }) => {
      ...
      if (!result.success) {
        socket.emit('error', { message: result.error || 'Invalid action', code: result.error || 'UNKNOWN' });
        return;
      }
      ...
      if (result.handComplete) {
        ...
        const showdown = game.players
          ...
          .map(p => ({
            playerId: p.id,
            cards: p.cards,
            handRank: result.winners?.find(w => w.playerId === p.id)?.handRank || ''
          }));
          ...
          setTimeout(() => {
            ...
            if (updatedGame && updatedGame.status === 'playing') {
              for (const p of updatedGame.players) {
                ...
                if (pSocketId) {
                  const pState = GameService.getGameStateForPlayer(updatedGame, p.id);
                  io.to(pSocketId).emit('game_state', pState);
                }
              }
              
              const currentPlayer = updatedGame.players[updatedGame.currentPlayerIndex];
              const validActions = GameService.getValidActions(updatedGame, currentPlayer);
              io.to(gameId).emit('player_turn', { 
                playerId: currentPlayer.id, 
    ...
    socket.on('reconnect_game', ({ gameId, playerId }) => {
      ...
      if (!result.success) {
        socket.emit('error', { message: 'Failed to reconnect', code: 'RECONNECT_FAILED' });
        return;
      }
      ...
      const gameState = GameService.getGameStateForPlayer(result.game!, playerId);
      socket.emit('game_state', gameState);
    });
